...
import { HumidityIcon, SunriseIcon, VisibilityIcon, WindIcon, SunsetIcon } from './components/Icons';
...
const App = () => {
  ...
  const API_KEY = 'bd42b5db816a769c755b02b13a69222c'
  ...
  const handleSearch = async (e) => {
    e.preventDefault();
    if (!city.trim()) setError("Please enter a valid city name.");
      ...
      <div className='flex items-center justify-center p-6 min-h-screen'>
        ...
        {!weather ? (
          <form onSubmit={handleSearch} className='flex flex-col relative'>
            <input value={city} onChange={(e) => setCity(e.target.value)} placeholder='Enter City or Country (min 3 characters)' 
            ...
            { suggestion.length > 0 && (
              <div className='absolute top-12 left-0 right-0 br-transparent shadow-md rounded z-10'>
                {suggestion.map((s) => (
                  <button type='button' key={`${s.lat}-${s.lon}`}
                  onClick={() => fetchWeatherData(`https://api.openweathermap.org/data/2.5/weather?lat=${s.lat}&lon=${s.lon}&appid=${API_KEY}&units=metric`,
          ...
          <div className='mt-6 text-center transition-opacity duration-500'>
            ...
            {weather && weather.weather && Array.isArray(weather.weather) && weather.weather[0] && (
  <>
    <img
